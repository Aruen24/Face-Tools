...
def train_one_epoch(data_loader, model, head, optimizer, criterion, kd_criterion, cur_epoch, ce_loss_meter, kd_loss_meter, conf):
    """Tain one epoch by traditional training.
    """
    for batch_idx, (images, labels) in enumerate(data_loader):
        images = images.to(conf.device).half()
        kd_features = F.normalize(labels[:,1:]).float().to(conf.device)
        # labels = labels.to(conf.device)
        labels = labels[:,0].long().to(conf.device)
        ...
        if conf.head_type == 'AdaM-Softmax':
            ...
            loss_kd = kd_criterion(pre_features.float(), kd_features.float()) / images.shape[0]
            new_loss = loss + loss_kd * 5
        ...
        kd_loss_meter.update((loss_kd * 5).item(), images.shape[0])
        
        if batch_idx % conf.print_freq == 0:
            loss_ce = ce_loss_meter.avg
            loss_kd = kd_loss_meter.avg
            
            lr = get_lr(optimizer)
            logger.info('Epoch %d, iter %d/%d, lr %f, loss_ce %f loss_kd %f' % 
                        (cur_epoch, batch_idx, len(data_loader), lr, loss_ce, loss_kd))
...
def train(conf):
    ...
    """
    trainset = MXFaceDataset(root_dir=conf.data_root)
    ...
    model = torch.nn.DataParallel(model.half(), device_ids=[0,1])
    ...
    for epoch in range(ori_epoch, conf.epoches):
        ...
        train_one_epoch(data_loader, model, head, optimizer, 
                        criterion, kd_criterion, epoch, ce_loss_meter, kd_loss_meter, conf)
        lr_schedule.step()                        
