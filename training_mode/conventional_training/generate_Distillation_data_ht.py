...
from backbone import hutian_resnet100
from backbone import hutian_resnet100_mask
...
def image_encode(args, i, item, q_out):
    ...
    if len(item) > 3 and args.pack_label:
        ...
        header = mx.recordio.IRHeader(0, [item[2]]+item[3], item[0], 0)
        #print([item[2]]+item[3])
        #print('************************')
    else:
        header = mx.recordio.IRHeader(0, item[2], item[0], 0)
...
if __name__ == '__main__':
    args = parse_args()
    # 首先加载网络模型
    model = hutian_resnet100_mask.iresnet100(pretrained=False, if_softmax = False, if_l2_norm = False)
    pre_dict = torch.load("/home/mapengfei/data/FR/vargfacenet/save/mask_r100_arcface_no_kd_298424/Epoch_34.pt")
    ...
    model = model.cuda()
    model = torch.nn.DataParallel(model.half(), device_ids=[0,1,2,3,4,5])
            ...
            if fname.startswith(args.prefix) and fname.endswith('.lst'):
                ...
                if args.num_thread > 1 and multiprocessing is not None:
                    ...
                    for i, item in enumerate(image_list):
                        fullpath = os.path.join(args.root, item[1])
                        # print(item,item[1])
                        img_mat = single_input(fullpath)
                        imgs.append(img_mat)
                        items.append(item)
                        if (i+1) % batch_size == 0 or i == img_num-1:
                            # pyTorch推理
                            model.eval()
                            with torch.no_grad():
                                t_imgs = torch.from_numpy(np.array(imgs))
                                t_imgs = t_imgs.cuda().half()
                                features = model(t_imgs)
                                features = features.data.cpu().numpy()
                                features = features.tolist()
                            for k in range(len(features)): 
                                items[k] += [features[k]]
                                #print('iiii ', i-len(features)+k+1)
                                q_in[(i-len(features)+k+1) % len(q_in)].put(((i-len(features)+k+1), items[k]))
                            imgs.clear()
                            items.clear()
                    for q in q_in:
                        q.put(None)
                    ...
                    for i, item in enumerate(image_list):
                        ...
                        # model.forward(Batch([mx.nd.array(img_mat)]), is_train=False)
                        # feature = model.get_outputs()[0].asnumpy().tolist()
