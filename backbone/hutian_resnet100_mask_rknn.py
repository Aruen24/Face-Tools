...
class GAP_block(nn.Module):
    def __init__(self, in_c):
        ...
        self.gap_mask_rknn = torch.nn.AdaptiveAvgPool2d((1, 1))
        self.bn = nn.BatchNorm2d(num_features=in_c, eps=2e-5, momentum=0.9, affine=True)
    def forward(self, x):
...
class IResNet(nn.Module):
    ...
    def __init__(self,
        ...
        # self.GDC_mask = GDC_block(in_c = 512 * block.expansion, out_c = 512 * block.expansion,
        #                     kernel = (5, 9), stride = (1, 1), padding = (0, 0), groups = 512 * block.expansion)
        self.GAP_mask = GAP_block(in_c = 512 * block.expansion)
...
def _iresnet(arch, block, layers, pretrained, if_softmax = False, if_l2_norm = False, classes = 100, progress=True, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_dict = torch.load('/home/mapengfei/.cache/torch/checkpoints/iresnet100-73e07ba7.pth')
